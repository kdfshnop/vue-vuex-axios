<template>
    <CollapsePanel class="gap-2" :expand="expand">    
            ...
            <el-form-item label="代理区域">    
                <ul class="region">
                    <li v-for="(region, index) in regions" :key="index">
                        <!--<span v-for="(s,i) in region.label" :key="i">{{s}}</span>-->
                        <span>{{region.label}}</span>
                        <el-button type="text" class="remove" @click="handleRemove(index)" v-show="mode=='create' || mode=='edit' && status=='editing'"><i class="el-icon-error"></i></el-button>
                    </li>
                </ul>                                        
            </el-form-item>                                                         
        ...
        <el-dialog title="添加代理区域" :visible.sync="visible">
            ...
            <territory ref="region" v-model="selectedRegion" :startLevel='2' :endLevel="4" :placeholder="'请输入城市或区域名'"></territory>
...
export default {
    name: "agentArea",
    props: ['item', 'mode'],
    components: {CollapsePanel, Region,Territory},
    data() {
        return {
            expand: true,
            visible: false,
        ...
        handleAddRegionConfirm() {
            if(this.mode=='create') {
                ...
                tmp.push({
                        ...
                        label: this.selectedRegion.name
                    });            
