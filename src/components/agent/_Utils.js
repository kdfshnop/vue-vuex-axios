...
export function generateParam(state) {
    ...
    agency.cityId = safeGet(state, "AgentBasicInfo.agentCity.id.1");
    ...
    safeGet(state, "AgentArea.regions").forEach((r)=>{
        agencyRegions.push({
            ...
            //createTime: ''// 创建接口不需要传递
            //   id: ""// 创建接口不需要传递
            level: r.level || (['', '', 3, 4])[r.val.id.length-1],// 区域级别，1：国家，2：省/直辖市，3：地级市，4：区、县、县级市，5：片区/板块
            //operator: ''
            regionId: r.regionId || r.val.id[2] || null,// 根据level不同传递不同的意义的值，在创建接口中可以，但是在详情接口中不行
            provinceId: r.provinceId || r.val.id[0],
            cityId: r.cityId || r.val.id[1], 
            id: r.id           
            //status: '' // 
          //   updateTime: ''
        });
