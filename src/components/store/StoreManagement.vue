...
<template>
    <el-container>
        <el-main>
            ...
            <el-form ref="form" :model="form" label-width="180px" class="gap">
                <el-row >
                    <el-col :span="12">
                        ...
                        <el-form-item label="门店所属城市" prop="areaInfo">
                            <territory v-model="form.areaInfo" :startLevel='startLevel' :endLevel="endLevel"></territory>
                        </el-form-item>
            ...
            <div class="search-result">共搜索到 {{summary.storeTotal}}家门店，{{summary.hASTotal}}家有代理商，{{summary.nASTotal}}家无代理商</div>
            <el-table :data="storeInfo.data" border style="width: 100%">
                ...
                <el-table-column label="操作" width="300px" align="center">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="browserStore(scope.$index, scope.row)" type="text">详情 </el-button>
            ...
            <!--二维码对话框-->
                <el-dialog title="门店二维码" :visible.sync="qrCodeShow" width="40%" v-if="qrCodeInfo.length">
                    <div style="text-align:center;">
                        <img :src="qrCodeInfo[2].url" alt="" v-if="!checked" style="width:60%;">
                        <img :src="qrCodeInfo[4].url" alt="" v-if="checked" style="width:60%;">
                        <div>{{qrCodeInfo[0].description}}</div>
                        <el-checkbox v-model="checked">是否添加悟空logo</el-checkbox>
                        <div v-if="!checked">
                            <el-button type="primary" @click="noLogo1">大尺寸下载1000*1000像素</el-button>
                            <el-button type="primary" @click="noLogo2">中尺寸下载500*500像素</el-button>
                            <el-button type="primary" @click="noLogo3">小尺寸下载300*300像素</el-button>
                        </div>
                        <div v-if="checked">
                            <el-button type="primary" @click="hasLogo1">大尺寸下载1000*1000像素</el-button>
                            <el-button type="primary" @click="hasLogo2">中尺寸下载500*500像素</el-button>
                            <el-button type="primary" @click="hasLogo3">小尺寸下载300*300像素</el-button>
                        </div>
                    </div>
                </el-dialog>
            <!--终止合作对话框-->
...
export default {
    ...
    components:{BreadCrumb,EditorStore,Region,Territory},
    ...
    methods:{
        ...
        requestList(){
            let self=this;
            // 判断是否选择了省市;
            if(JSON.stringify(this.form.areaInfo&&this.form.areaInfo) !== "{}"){
                this.form.cityId=this.form.areaInfo.id[1];
            };
            let realForm=Object.assign({},this.form);
            delete realForm.areaInfo;
            // 获取代理商列表信息;
        ...
        // 笨方法
        noLogo1(){
            window.open(this.qrCodeInfo[0].url);
        },
        noLogo2(){
            window.open(this.qrCodeInfo[1].url);
        },
        noLogo3(){
            window.open(this.qrCodeInfo[2].url);
        },
        hasLogo1(){
            window.open(this.qrCodeInfo[3].url);
        },
        hasLogo2(){
            window.open(this.qrCodeInfo[4].url);
        },
        hasLogo3(){
            window.open(this.qrCodeInfo[5].url);
        }
    }
}
</script>
